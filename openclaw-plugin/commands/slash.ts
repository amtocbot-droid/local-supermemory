...
export function registerStubCommands(api: OpenClawPluginApi): void {
	api.registerCommand({
		...
		handler: async () => {
			return {
				text: "Local Supermemory not configured. Ensure the local server is running.",
			}
		},
	})
	...
	api.registerCommand({
		...
		handler: async (ctx: { args?: string }) => {
			...
			try {
				...
				return { text: "Failed to save memory. Is the local server running?" }
			}
		},
	})
	...
	api.registerCommand({
		...
		handler: async (ctx: { args?: string }) => {
			...
			try {
				...
				return { text: "Failed to search memories. Is the local server running?" }
			}
		},
	})
}